...
이제 그럴듯하게 회전이 완성되었으니, 정육면체 같은 게 아니라 좀 더 복잡한 모델을 출력해보기로 했다. 3D 모델 관련 파일 포맷들을 몇 개 살펴보았는데, 일단 무료 3D 툴인 [블렌더](https://www.blender.org/)를 설치하고 실행해서, 익스포트 가능한 포맷들을 보니 아래와 같았다.
...
일단은 여기까지 하고 나중에 또 시간 날 때 텍스쳐 매핑이나 조명 등 여러가지를 한 번 구현해보기로 했다. 그 전에 마지막으로 가볍게 단순히 색상을 채워넣기만 해보기로 했다. PyQt 의 drawConvexPolygon 을 써서 그냥 랜덤한 색으로 채워넣는 것만 해봤더니 아래와 같았다.
...
어차피 다음에 텍스쳐 매핑을 구현하려면 각 다각형을 래스터화하고 픽셀당 그리기를 해야하는데, 그 때는 각 픽셀당 깊이 값으로 체크를 해서 그릴지 안 그릴지를 선택할 수 있어서 해결할 수 있을 문제같으므로 지금은 이 정도로 하고 보류하기로 했다.
